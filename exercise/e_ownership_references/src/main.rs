...
fn inspect(x: &String) {
    if x.ends_with("s") {
        println!("The String you entered is plural");
    } else {
        println!("The String you entered is singular");
    }
}

fn change(y: &mut String) {
    if !y.ends_with("s") {
        y.push_str("s");
    }
}

fn eat(z: String) -> bool {
    z.starts_with("b") && z.contains("a")
}
